...
class mod_feedback_templates_table extends flexible_table {
    ...
    public function display($templates) {
        global $OUTPUT;
        if (empty($templates)) {
            echo $OUTPUT->notification(get_string('no_templates_available_yet', 'feedback'), notification::NOTIFY_INFO, false);
            return;
        }
     ...
     * @return action_menu
     */
    private function get_row_actions(stdClass $template) {
        global $PAGE, $OUTPUT;
