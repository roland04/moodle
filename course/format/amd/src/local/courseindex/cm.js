...
export default class Component extends DndCmItem {
    ...
    stateReady(state) {
        ...
        if (!cm.uservisible || !cm.url) {
            ...
            if (!document.getElementById(cm.anchor)) {
                element.setAttribute('href', this._getActivityCourseURL(cm));
            }
        }
    }
    ...
    _activityAnchor(event) {
        ...
        window.location = this._getActivityCourseURL(cm);
    }
    ...
    _getActivityCourseURL(cm) {
        ...
        return `${course.baseurl}&section=${section.number}#${cm.anchor}`;
    }
}
