...
class url_select implements renderable, templatable {
     ...
     * @param array $nothing
     * @param null $formid
     * @param null $showbutton Set to text of button if it should be visible
     *   or null if it should be hidden (hidden version always has text 'go')
     * @param array|null $disabledoptions array of disabled options if any. Disabled option are defined by
    ...
    public function enable_option($url) {
        if (array_key_exists($url, $this->urls)) {
            if (isset($this->disabledoptions[$url])) {
